...
Launch:
  ...
  Terminal 4: ros2 run px4_offboard apriltag_precision_lander --ros-args -p target_tag_id:=0
═══════════════════════════════════════════════════════════════════
"""
...
class Config:
    ...
    # ── AprilTag ───────────────────────────────────────────────
    APRILTAG_DICT = aruco.DICT_APRILTAG_25h9
    MARKER_SIZE = 0.50      # metres, must match apriltag_* model.sdf
    ...
    SEARCH_POS = TAG_POSITIONS[TARGET_TAG_ID]
    ...
    DETECT_UPSCALE = 2.0
    SEARCH_ACCEPT_RADIUS = 0.45
    SEARCH_PATTERN_STEP = 1.0
    SEARCH_PATTERN_MAX_RADIUS = 5.0
    SEARCH_PATTERN_DWELL = 1.2
...
class AprilTagPrecisionLander(Node):
    ...
    def __init__(self):
        ...
        cfg.SEARCH_POS = cfg.TAG_POSITIONS[self.target_tag_id]
        ...
        # ── Camera / AprilTag setup ───────────────────────────────
        self.bridge = CvBridge()
        aruco_dict = aruco.getPredefinedDictionary(cfg.APRILTAG_DICT)
        ...
        self.get_logger().info('🎯 AprilTagPrecisionLander ready — state: INIT')
        self.get_logger().info(f'   Target tag id: {self.target_tag_id}')
        self.get_logger().info(f'   Tag search position: x={cfg.SEARCH_POS[0]}m, y={cfg.SEARCH_POS[1]}m')
        ...
        try:
            ...
            if ids is not None and len(ids) > 0:
                ...
                for i, marker_id in enumerate(ids_flat):
                    ...
                    self.detected_tags[marker_id] = {
                        'center_px': corners[i].reshape((-1, 2)).mean(axis=0),
                        'rvec': rvec,
                        'tvec': tvec,
                        'time': now,
                    }
                ...
                idx = int(matches[0])
                detected = self.detected_tags[self.target_tag_id]
                rvec = detected['rvec']
                tvec = detected['tvec']
                self.marker_tvec = tvec  # [x, y, z] in camera frame
                self.marker_rvec = rvec
                self.marker_center_px = detected['center_px']
                self.marker_id = int(ids_flat[idx])
    ...
    def _detect_markers(self, gray: np.ndarray):
        """Detect AprilTags, with an upscaled fallback for small/blurred tags."""
        corners, ids, _ = self.detector.detectMarkers(gray)
        if ids is not None or Config.DETECT_UPSCALE <= 1.0:
            return corners, ids

        scale = Config.DETECT_UPSCALE
        upscaled = cv2.resize(gray, None, fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)
        corners, ids, _ = self.detector.detectMarkers(upscaled)
        if ids is None:
            return corners, ids

        scaled_corners = [corner / scale for corner in corners]
        return scaled_corners, ids
    ...
    def _estimate_marker_pose(self, corners: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if not success:
            raise RuntimeError('solvePnP failed for detected AprilTag marker')
    ...
    def _log_search_diagnostics(self):
        ...
        if self.camera_frame_count == 0:
            self.get_logger().warn(
                'No /gimbal_camera frames received. Check the ros_gz_bridge command uses '
                '/world/apriltag_landing/...:=/gimbal_camera')
            return

        frame_age = now - self.last_camera_frame_time
        if frame_age > 1.0:
            self.get_logger().warn(
                f'/gimbal_camera is stale ({frame_age:.1f}s since last frame). Check Terminal 3 bridge.')
            return

        if self.visible_marker_ids:
            self.get_logger().warn(
                f'Searching for tag {self.target_tag_id}, but visible tags are {self.visible_marker_ids}')
        else:
            self.get_logger().warn(
                f'Camera frames received ({self.camera_frame_count}), but no AprilTag 25h9 markers detected')

    def _state_horizontal_approach(self):
        """PX4 phase 1: move horizontally over the target while holding altitude."""
        cfg = Config

        target_visible = self._marker_recent()
        if target_visible:
        ...
        if target_visible and self.target_rel_norm < cfg.PLD_HACC_RAD:
    ...
    def _state_target_lost(self):
        ...
        if self._marker_recent() and self._update_target_from_vision():
    ...
    def _reset_search_pattern(self, anchor_xy: np.ndarray):
        ...
        self.target_abs = np.array([
            ...
            self._search_anchor[1],
            Config.LANDING_TARGET_Z,
        ])

    def _search_offset(self) -> np.ndarray:
        if self._search_wp_index <= 0:
            return np.array([0.0, 0.0])

        cfg = Config
        ring = 1 + ((self._search_wp_index - 1) // 8)
        radius = min(cfg.SEARCH_PATTERN_STEP * ring, cfg.SEARCH_PATTERN_MAX_RADIUS)
        point = (self._search_wp_index - 1) % 8
        offsets = [
            ( radius, 0.0),
            ( radius, radius),
            ( 0.0, radius),
        ...
        return np.array(offsets[point], dtype=float)

    def _run_search_pattern(self):
        cfg = Config
        target_xy = self._search_anchor + self._search_offset()
        self.target_abs = np.array([target_xy[0], target_xy[1], cfg.LANDING_TARGET_Z])
        self.target_rel_xy = self.target_abs[:2] - self.pos[:2]
        self.target_rel_norm = float(np.linalg.norm(self.target_rel_xy))
        self.sp = self._step_setpoint_to_target(-cfg.CRUISE_ALT, cfg.MAX_APPROACH_STEP)

        if self._target_counter % cfg.CTRL_HZ == 0:
            self.get_logger().info(
                f'🔎 Search pattern: waypoint={self._search_wp_index}, '
                f'target=[{target_xy[0]:.2f}, {target_xy[1]:.2f}], '
                f'error={self.target_rel_norm:.2f}m')

        if self.target_rel_norm < cfg.SEARCH_ACCEPT_RADIUS:
            if self._search_wp_arrival_time is None:
                self._search_wp_arrival_time = time.time()
            elif time.time() - self._search_wp_arrival_time >= cfg.SEARCH_PATTERN_DWELL:
                self._search_wp_index += 1
                self._search_wp_arrival_time = None
        else:
            self._search_wp_arrival_time = None
    ...
    def _image_center_rel_ned(
        self,
        center_px: Optional[np.ndarray],
        max_correction: float,
        log_rejection: bool = True,
    ) -> Optional[np.ndarray]:
        """Estimate ground offset from image-center error for a downward camera."""
    ...
    def _marker_image_rel_ned(self) -> Optional[np.ndarray]:
        """Estimate selected marker ground offset from image-center error."""
