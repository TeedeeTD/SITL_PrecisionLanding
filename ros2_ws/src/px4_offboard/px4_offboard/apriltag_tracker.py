...
detects the target AprilTag using OpenCV, estimates its 3D pose,
performs body-to-ENU coordinate transformations, and publishes:
1. /apriltag_tracker/pose (geometry_msgs/msg/PoseStamped)
...
class AprilTagTrackerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("pose_output_topic", "/apriltag_tracker/pose")
        self.declare_parameter("annotated_image_topic", "/landing/annotated_image")
        self.declare_parameter("dictionary", "DICT_APRILTAG_25h9")
        ...
        self.get_logger().info(
            f"AprilTag Tracker Node ready: dict={dict_name}, tag_id={self.target_tag_id}, size={self.marker_size}m"
        )
    ...
    def _on_image(self, msg: Image):
        ...
        if self.cam_mtx is None:
            ...
            # Nominal HFOV = 1.4137 rad (81 degrees)
            fx = (w / 2.0) / math.tan(1.4137 / 2.0)
