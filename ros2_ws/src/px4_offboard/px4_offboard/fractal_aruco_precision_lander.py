#!/usr/bin/env python3
"""Fractal ArUco precision landing for PX4 x500_gimbal.

This node is intentionally Fractal-only. Detection and pose estimation are done
by the C++ aruco_fractal_tracker node; this node consumes PoseStamped tracker
poses, keeps all landing logic in ENU, and converts to PX4 NED only at the
TrajectorySetpoint/MAVLink boundary.
"""
...
class FractalConfig:
    ...
    # Dynamic acceptance gates. At 15m the tracker pose is visibly noisy, so
    # the controller accepts a wider cone and tightens as altitude decreases.
    ...
    # Tracker pose axes. The tracker publishes camera optical coordinates:
    # x=image-right, y=image-down, z=forward into the scene. With the gimbal
    # looking straight down and yaw-stabilized, image-right maps to ENU East and
    # image-down maps to ENU South.
    CAMERA_X_TO_ENU_EAST_SIGN = 1.0
    CAMERA_Y_TO_ENU_NORTH_SIGN = -1.0
    CAMERA_YAW_FRAME = "local"  # "local" or "body"
...
class FractalArucoPrecisionLander(Node):
    def __init__(self) -> None:
        super().__init__("fractal_aruco_precision_lander")
        self.cfg = FractalConfig
        ...
        self.create_subscription(
            VehicleLandDetected, "/fmu/out/vehicle_land_detected", self._on_land_detected, px4_qos
        )
        self.create_subscription(PoseStamped, self.pose_topic, self._on_tracker_pose, 10)
        ...
        self.sp_ned = np.zeros(3, dtype=float)
        self.sp_yaw = 0.0
        ...
        self.target_ned: Optional[np.ndarray] = None
        ...
        self.get_logger().info("Fractal ArUco PrecisionLander ready - state: INIT")
        ...
        self.get_logger().info(f"Fractal tracker pose topic: {self.pose_topic}")
        self.get_logger().info(
    ...
    def _load_parameters(self) -> None:
        ...
        if self.search_frame == "enu":
            ...
            self.search_ned_xy = self._enu_xy_to_ned_xy(self.search_enu_xy)
        elif self.search_frame == "ned":
            self.search_ned_xy = self.search_input_xy.copy()
            self.search_enu_xy = self._ned_xy_to_enu_xy(self.search_ned_xy)
        else:
    ...
    def _init_mavlink(self) -> None:
        try:
            ...
            self.mav_conn = mavutil.mavlink_connection(
                f"udpout:{self.cfg.MAVLINK_HOST}:{self.cfg.MAVLINK_PORT}",
                dialect="common",
            )
            self.get_logger().info(
                f"MAVLink connected: udpout:{self.cfg.MAVLINK_HOST}:{self.cfg.MAVLINK_PORT}"
            )
        except ImportError:
    ...
    def _on_position(self, msg: VehicleLocalPosition) -> None:
        self.pos_ned = np.array([msg.x, msg.y, msg.z], dtype=float)
    ...
    def _on_attitude(self, msg: VehicleAttitude) -> None:
        self.q_att = np.array(msg.q, dtype=float)
    ...
    def _on_land_detected(self, msg: VehicleLandDetected) -> None:
        ...
        self.maybe_landed = bool(msg.maybe_landed)
        self.at_rest = bool(msg.at_rest)
    ...
    def _on_tracker_pose(self, msg: PoseStamped) -> None:
        ...
        self.target_ned = np.array(
            [target_ned_xy[0], target_ned_xy[1], self.cfg.LANDING_TARGET_Z],
            dtype=float,
        )
    ...
    def _state_init(self) -> None:
        self.sp_ned = np.array([0.0, 0.0, 0.0], dtype=float)
        self.warmup_count += 1
    ...
    def _state_takeoff(self) -> None:
        if not (self.armed and self.offboard_active):
            self.sp_ned = np.array([0.0, 0.0, -self.cruise_alt], dtype=float)
            self.warmup_count += 1
        ...
        self.sp_ned = np.array([0.0, 0.0, -self.cruise_alt], dtype=float)
        if abs(self.pos_ned[2] + self.cruise_alt) < 0.35:
            self.get_logger().info(
                f"Takeoff complete at {-self.pos_ned[2]:.1f}m - pitching gimbal down"
            )
            self.warmup_count = 0
    ...
    def _state_fly_to_search(self) -> None:
        self.sp_ned = np.array(
            [self.search_ned_xy[0], self.search_ned_xy[1], -self.cruise_alt],
            dtype=float,
        )
        dist = float(np.linalg.norm(self.pos_ned[:2] - self.search_ned_xy))
        if dist < 0.60:
    ...
    def _state_horizontal_approach(self) -> None:
        ...
        self.sp_ned = self._visual_setpoint(hold_z, self.max_align_step)
        ...
        if self._target_counter % self.ctrl_hz == 0:
            self._log_guidance("Fractal visual align", align_radius)
        self._target_counter += 1

        if self._centered_count >= self.align_confirm_count:
            self.get_logger().info(
                f"Fractal target inside dynamic align gate ({align_radius:.2f}m) - descending"
            )
            self._target_counter = 0
            self._centered_count = 0
            self._descent_drift_count = 0
            self._descent_z_sp = float(self.pos_ned[2])
    ...
    def _state_descend_over_target(self) -> None:
        if not self._pose_recent():
            self._start_target_lost(
                "fractal target lost during visual descent",
        ...
        if descent_allowed:
            ...
            self._descent_z_sp = float(self.pos_ned[2])
            self.get_logger().warn(
                f"Fractal descent paused for realign: err={self.target_rel_norm:.2f}m "
        ...
        self.sp_ned = self._visual_setpoint(self._descent_z_sp, self.max_descent_step)

        if self._target_counter % self.ctrl_hz == 0:
            phase = "descending" if descent_allowed else "align-hold"
            self._log_guidance(f"Fractal visual descent ({phase})", descent_radius)
        self._target_counter += 1

        if -self.pos_ned[2] <= self.final_alt + 0.05:
            self.get_logger().info(
                f"Fractal final altitude reached ({self.final_alt:.2f}m) - switching to PX4 land"
            )
            self._transition("LAND")
    ...
    def _state_target_lost(self) -> None:
        ...
        if self._pose_recent():
            ...
            self.get_logger().info(f"Fractal target reacquired - resuming {resume}")
        ...
        if elapsed < self.cfg.TARGET_LOSS_GRACE:
            hold_xy = self.target_ned[:2] if self.target_ned is not None else self.search_ned_xy
            self.sp_ned = np.array([hold_xy[0], hold_xy[1], float(self.pos_ned[2])], dtype=float)
            if self._target_counter % self.ctrl_hz == 0:
                self.get_logger().warn(
                    f"Fractal target temporarily lost - holding last visual setpoint "
        ...
        dist = float(np.linalg.norm(self.pos_ned[:2] - self.search_ned_xy))
        alt_err = abs(self.pos_ned[2] + self.cruise_alt)
        if self._target_counter % self.ctrl_hz == 0:
            self.get_logger().warn(
                f"Fractal target lost - returning to search pose: dist={dist:.2f}m "
    ...
    def _state_land(self) -> None:
        if self.target_ned is None:
            self.target_ned = np.array(
                [self.search_ned_xy[0], self.search_ned_xy[1], self.cfg.LANDING_TARGET_Z],
                dtype=float,
            )
        ...
        near_ground = self.pos_ned[2] > -(self.final_alt + 0.08)
        ...
        if now - self._last_land_wait_log > self.cfg.LAND_WAIT_WARN_INTERVAL:
            ...
            self.get_logger().warn(
                ...
                f"alt={-self.pos_ned[2]:.2f}m, near_ground={near_ground}, "
    ...
    def _current_enu_xy(self) -> np.ndarray:
        return self._ned_xy_to_enu_xy(self.pos_ned[:2])
    ...
    def _ned_sp_to_enu_text(self, ned_sp: Optional[np.ndarray] = None) -> str:
        sp = self.sp_ned if ned_sp is None else ned_sp
    ...
    def _altitude(self) -> float:
        return max(0.0, -float(self.pos_ned[2]))
    ...
    def _visual_setpoint(self, target_z: float, max_step: float) -> np.ndarray:
        if self.filtered_rel_enu is None:
            hold_xy = self.target_ned[:2] if self.target_ned is not None else self.pos_ned[:2]
        ...
        target_ned_xy = self._enu_xy_to_ned_xy(target_enu_xy)
        return np.array([target_ned_xy[0], target_ned_xy[1], target_z], dtype=float)

    def _send_current_landing_target(self) -> None:
        if self.target_ned is None:
            return

        rel = self.target_ned[:2] - self.pos_ned[:2]
        rel_z = max(-float(self.pos_ned[2]), self.final_alt)
        self._send_landing_target(float(rel[0]), float(rel[1]), rel_z)

    def _send_landing_target(self, rel_x: float, rel_y: float, rel_z: float) -> None:
        if self.mav_conn is None:
            return

        try:
            if self.target_ned is not None:
                target_x = float(self.target_ned[0])
                target_y = float(self.target_ned[1])
                target_z = float(self.target_ned[2])
            else:
                target_x = float(self.pos_ned[0] + rel_x)
                target_y = float(self.pos_ned[1] + rel_y)
                target_z = float(self.pos_ned[2] + rel_z)
            ...
            self.mav_conn.mav.landing_target_send(
                ...
                0.0,
                0.0,
                float(math.sqrt(rel_x * rel_x + rel_y * rel_y + rel_z * rel_z)),
                1.0,
                1.0,
                target_x,
                target_y,
            ...
            self.get_logger().warn(f"MAVLink LANDING_TARGET send error: {exc}", throttle_duration_sec=2.0)
    ...
    def _log_search_diagnostics(self) -> None:
        ...
        if self.pose_count == 0:
            self.get_logger().warn(
                ...
                "Check aruco_fractal_tracker, camera bridge, and rqt annotated image."
            )
            return
    ...
    def _log_guidance(self, label: str, gate: float) -> None:
        ...
        target_enu = (
            self._ned_xy_to_enu_xy(self.target_ned[:2])
            if self.target_ned is not None
        ...
        self.get_logger().info(
            ...
            f"sp_enu={self._ned_sp_to_enu_text()} gain={self._servo_gain():.2f} "
    ...
    def _pub_setpoint(self) -> None:
        ...
        msg.position = [float(v) for v in self.sp_ned]
...
def main(args=None) -> None:
    ...
    node = FractalArucoPrecisionLander()
    try:
        rclpy.spin(node)
