#!/usr/bin/env python3
"""Fractal ArUco precision landing for PX4 x500_gimbal using MAVROS.

This node consumes PoseStamped tracker poses from the C++ tracker,
...
class FractalArucoPrecisionLander(Node):
    def __init__(self) -> None:
        ...
        # We subscribe to state and extended state with transient local / reliable QoS
        ...
        self.get_logger().info("Fractal ArUco PrecisionLander (MAVROS) ready - state: INIT")
    ...
    def _send_current_landing_target(self) -> None:
        ...
        # MAVROS LandingTarget expects coordinates in LOCAL_NED when frame is LOCAL_NED.
        # Convert absolute target position:
        target_x = float(self.target_enu[1]) # North
        target_y = float(self.target_enu[0]) # East
        target_z = float(-self.target_enu[2]) # Down

        # Convert relative target vector:
    ...
    def _pub_setpoint(self) -> None:
        ...
        # Convert sp_yaw (rotation around Z axis) to quaternion
        msg.pose.orientation.x = 0.0
        msg.pose.orientation.y = 0.0
    ...
    def _camera_xy_to_local_enu(self, camera_xy: np.ndarray) -> np.ndarray:
        ...
        # East = X_body * cos(yaw) - Y_body * sin(yaw)
        # North = X_body * sin(yaw) + Y_body * cos(yaw)
